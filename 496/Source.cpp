...
class Solution
{
	static void quick_remove(vector<int>& v, size_t i)
	{
		if (i != v.size() - 1)
			v[i] = v.back();
		v.pop_back();
...
public:
	vector<int> nextGreaterElement(vector<int>& nums1, vector<int>& nums2)
	{
		...
		unordered_map<int, size_t> reverse_table;
		for (size_t i = 0; i < nums1.size(); i++)
			reverse_table[nums1[i]] = i;
		priority_queue<int,vector<int>,std::greater<int>> buffer;
		for (auto n : nums2)
		{
			while (!buffer.empty() && n > buffer.top())
			{
				result[reverse_table.at(buffer.top())] = n;
				buffer.pop();
			}
			if (reverse_table.count(n) != 0)
				buffer.push(n);
		}
		return result;
	}
...
void test(vector<int> num1, vector<int> num2)
{
	...
	cout << "Output: " << s.nextGreaterElement(num1, num2) << endl;
}
